...
exports.handler = async function (event) {
  // 백그라운드 함수: Netlify가 즉시 202를 반환하고, 이 코드는 최대 15분까지 실행됨
  ...
  try {
    ...
    var prompt = "";
    prompt += "의료인문학 강의('고통이란 무엇인가')에서 의대생의 인생 그래프를 해석하세요.\n\n";
    ...
    // Firebase에 AI 해석 결과 저장
    if (firebaseUrl && fbKey) {
      await firebasePut(firebaseUrl + "/submissions/" + fbKey + "/aiInterpretation.json", text);
    }
  } catch (err) {
    // 에러도 Firebase에 기록
    if (input && input.firebaseUrl && input.fbKey) {
      try {
        await firebasePut(input.firebaseUrl + "/submissions/" + input.fbKey + "/aiError.json", err.message);
      } catch (e) {}
    }
  }
